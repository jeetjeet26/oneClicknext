...
describe('forgestudio social callback linkedin route', () => {
  ...
  it('redirects with unauthorized error when the session is missing', async () => {
    ...
    const response = await GET(
      new NextRequest(
        'http://localhost/api/forgestudio/social/callback/linkedin?code=abc&state=some-state'
      )
    )
  ...
  it('redirects with invalid_state when signed state is invalid', async () => {
    ...
    const response = await GET(
      new NextRequest(
        'http://localhost/api/forgestudio/social/callback/linkedin?code=abc&state=invalid-state'
      )
    )
