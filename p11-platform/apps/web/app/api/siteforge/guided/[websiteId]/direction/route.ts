...
import { guidedDirectionEditRequestSchema } from "@/utils/siteforge/guided/contracts";
import {
...
export async function POST(
  ...
  try {
    ...
    const parsed = guidedDirectionEditRequestSchema.safeParse(
      await request.json(),
    );
    if (!parsed.success) {
      return NextResponse.json(
        { error: "Provide one valid creative direction edit request." },
        { status: 400, headers: ctx.responseHeaders },
      );
    }
    ...
    const result = await siteForgeGuidedService().editDirection(
      websiteId,
      parsed.data,
      user.id,
    );
    ctx.logSuccess(200, {
      ...
      outcome:
        "editOutcome" in result ? result.editOutcome?.outcome : undefined,
    });
