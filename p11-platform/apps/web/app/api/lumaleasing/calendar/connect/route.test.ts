...
const createSignedGoogleOAuthStateMock = vi.fn()
...
vi.mock('@/utils/services/google-oauth-state', () => ({
  createSignedGoogleOAuthState: createSignedGoogleOAuthStateMock,
}))

describe('Calendar connect route', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.resetModules()
    vi.stubEnv('GOOGLE_CLIENT_ID', 'google-client-id')
    vi.stubEnv('NEXT_PUBLIC_APP_URL', 'https://app.example.com')
    createSignedGoogleOAuthStateMock.mockReturnValue('signed-state')
  })
  ...
  it('returns 401 when unauthorized', async () => {
    ...
    const request = new Request(
      'http://localhost/api/lumaleasing/calendar/connect?propertyId=property-1'
    ) as NextRequest
  ...
  it('redirects to Google with a signed state payload', async () => {
    ...
    expect(response.headers.get('x-request-id')).toBeTruthy()
    expect(createSignedGoogleOAuthStateMock).toHaveBeenCalledWith({
  ...
  it('redirects Microsoft connects to same-origin unified OAuth start', async () => {
    ...
    const { GET } = await import('./route')
    const request = new Request(
      'https://current.example.com/api/lumaleasing/calendar/connect?propertyId=property-1&provider=microsoft'
    ) as NextRequest
    ...
    expect(redirectUrl.searchParams.get('capabilities')).toBe('calendar')
  })
})
