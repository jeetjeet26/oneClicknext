...
describe('marketvision brand-intelligence competitor route auth', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    fromMock.mockReset()
  ...
  it('GET returns 401 when unauthenticated', async () => {
    ...
    const response = await GET(
      makeNextRequest('http://localhost/api/marketvision/brand-intelligence/competitor-1'),
      { params: Promise.resolve({ competitorId: 'competitor-1' }) },
    )
